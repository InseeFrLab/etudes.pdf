...
#' @export
generer_tableau_insee <- function(n_lignes, format_colonnes = 1, nom_fichier = "tableau_insee.png") {
  
  # 1. Ajustement du nombre de colonnes
  nb_cols_data <- ifelse(format_colonnes == 1, 5, 10)
  df <- data.frame(Indicateur = paste("Mon indicateur sur une longue ligne", 1:n_lignes))
  for(i in 2:nb_cols_data) {
    df[[paste("Donnée", i-1)]] <- format(round(runif(n_lignes, 10, 100), 1), nsmall = 1, decimal.mark = ",")
  }
