...
-   [Charte Insee Flash 2025](reference/figures/Cahier_des_charges_IF_2025.pdf)
...
-   Lancer la fonction `install_insee_flash_typst()` pour installer l'extension typst dédiée aux Insee Flash ou `install_insee_analyses_typst()` pour installer l'extension typst des Insee Analyses: un dossier `\_extensions` est créé.

-   déplacer le fichier modèle `if157.qmd` ou `ia144.qmd`du dossier `\_extensions/insee-flash` ou `\_extensions/insee-analyses` vers la racine puis lancer `quarto render if157.qmd` ou `quarto render ia144.qmd`pour générer le pdf.
